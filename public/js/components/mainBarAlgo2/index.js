...
function MainBarSearchAlgo2(data) {
  ...
  recettes = datas.filter(function (e) {
    ...
    //   for (let j = 0; j < e.ustensils.length; j++) {
      ...
      let isMatchUstensils = false
      e.ingredients.forEach(ingredient => {
        if(ingredient.ingredient.match(".*" + valueMainBar.value.toLowerCase() + ".*")) {
          isMatchIngredient = true
        }
      });
      e.ustensils.forEach(ustensil => {
        if(ustensil.match(".*" + valueMainBar.value.toLowerCase() + ".*")) {
          isMatchUstensils = true
        }
      });
        if (
